...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="bg-slate-300">
        ...
        <h1 class="ml-96 mt-20 font-serif font-bold">Edit New List</h1>
        <div class="mt-3 ml-96">
        <form action="" method="POST">
            @csrf
            @method('PUT')
            <label class="px-5 font-bold">Name: </label>
            <input type="text" name="name" value="{{$list->name}}" class="w-72 h-12">
            <input type="submit" name="Update" value="Update" class="bg-blue-600  ml-4 mt-7 h-10 w-15 rounded-lg px-4 hover:bg-blue-500">
        </form>
    </div>
    </body>
</html>
